...
static void _do_prefix(vfs_mount_t *mountp, const char *name, char *buffer, size_t len)
{
    ...
    if (getcwd(buffer, len) == NULL) {
        puts("fs_native: can't get working directory");
        exit(errno);
    }

    len = sizeof(buffer) - strlen(buffer);
    assert(len > strlen(fs_desc->hostpath));

    size_t res = snprintf(&buffer[strlen(buffer)], len, "%s/%u/%s",
                          fs_desc->hostpath, _native_id, name);
    assert(res <= len);
}
