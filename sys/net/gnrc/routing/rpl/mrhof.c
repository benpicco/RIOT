 ...
 */
static netstats_nb_t *_mrhof_get_stats(gnrc_rpl_parent_t *parent)
{
    ...
    return netstats_nb_get(&iface->netif, nce.l2addr, nce.l2addr_len);
}
...
uint16_t calc_rank(gnrc_rpl_dodag_t *dodag, uint16_t base_rank)
{
    ...
    if (dodag == NULL) {
        ...
        uint16_t minhoprankincr = dodag->instance->min_hop_rank_inc;

        /* Determine the path cost through the preferred parent */
        elt_stats = _mrhof_get_stats(dodag->parents);
        uint16_t cost_rank = _mrhof_get_path_cost(dodag->parents, elt_stats);
        ...
        LL_FOREACH(dodag->parents, elt) {
            ...
            elt_stats = _mrhof_get_stats(elt);
...
int which_parent(gnrc_rpl_parent_t *p1, gnrc_rpl_parent_t *p2)
{
    ...
    netstats_nb_t *p1_stats = _mrhof_get_stats(p1);
    netstats_nb_t *p2_stats = _mrhof_get_stats(p2);
